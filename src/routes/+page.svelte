...
>
	{#each menuItems as m}
		<a
			href={m.href}
			class="col-span-1 flex h-full flex-col items-center justify-center gap-1 rounded-xl bg-green-300 px-2 py-5 font-semibold uppercase shadow-lg transition-transform duration-200 hover:scale-105 lg:py-10"
		>
			<svelte:component this={m.icon} size="50" />
			<p class="mt-1 w-20 lg:w-full lg:text-lg">{m.text}</p>
		</a>
	{/each}
</div>
{#if $loading === true}
